...
router.get('/:page', function(req, res) {
	...
	if (parseInt(req.params.page) > count/25 + 1){
		...
		if (req.query.s){		query.text = "(SELECT id as id, 'localities' as table, name, name_alt AS text, st_asgeojson(geom) AS geom FROM localities order by name limit 25 offset $1) union (SELECT id as id, 'landmarks' AS table, name, locality AS text, st_asgeojson(geom) AS geom FROM landmarks order by name limit 25 offset $1) ORDER BY name";
